...
Os testes padrão não exigem banco:
...
`TEST_DATABASE_URL` apontando para um banco descartável. Os testes limpam
...
A API single-tenant permite organizar links por campanha, tags e UTMs. A
listagem aceita filtros por texto (`q`), status, tag e campanha, e o dashboard
mostra métricas operacionais como cliques totais e último clique.

O painel web também oferece builder de UTM, QR code do link curto e exportação
